PRO cena_128t,cleanup=cleanup,ps_export=ps_export,recalculate_all=recalculate_all,export_images=export_images,version=version,$
...
IF N_Elements(channel) EQ 0 THEN channel=93
image_filter_fn='filter_uv_radial' ;applied ONLY to output images

IF StrLowCase(!version.os_family) EQ 'unix' THEN data_directory=rootdir('mwa')+filepath('',root='DATA',subdir=['128T','CenA']) $
    ELSE data_directory=rootdir('mwa')+filepath('',root='DATA3',subdir=['128T','CenA'])

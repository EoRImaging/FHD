PRO drift_june_128T,cleanup=cleanup,ps_export=ps_export,recalculate_all=recalculate_all,export_images=export_images,version=version,$
...
IF N_Elements(channel) EQ 0 THEN channel=117
image_filter_fn='' ;applied ONLY to output images

IF StrLowCase(!version.os_family) EQ 'unix' THEN data_directory=rootdir('mwa')+filepath('',root='DATA',subdir=['128T','Drift_June',Strn(channel)]) $
    ELSE data_directory=rootdir('mwa')+filepath('',root='DATA3',subdir=['128T','Drift_June',Strn(channel)])
...
general_obs,cleanup=cleanup,ps_export=ps_export,recalculate_all=recalculate_all,export_images=export_images,version=version,$
    ...
    complex_beam=complex_beam,double_precison_beam=double_precison_beam,FoV=FoV,no_ps=no_ps,transfer_mapfn=transfer_mapfn,_Extra=extra
!except=except
END
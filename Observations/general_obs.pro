PRO general_obs,cleanup=cleanup,recalculate_all=recalculate_all,export_images=export_images,version=version,$
    ...
    complex_beam=complex_beam,pad_uv_image=pad_uv_image,max_deconvolution_components=max_deconvolution_components,$
...
IF N_Elements(max_deconvolution_components) EQ 0 THEN max_deconvolution_components=10000. ;maximum total number of source components to fit
...
WHILE fi LT n_files DO BEGIN
    ...
    fhd_main,vis_file_list[fi],status_str,file_path_fhd=fhd_file_list[fi],n_pol=n_pol,recalculate_all=recalculate_all,$
        ...
        silent=silent,max_deconvolution_components=max_deconvolution_components,deconvolve=deconvolve,catalog_file_path=catalog_file_path,$
...
IF Keyword_Set(simultaneous) THEN BEGIN
    ...
    fhd_multi_wrap,fhd_file_list,status_arr,N_simultaneous=N_simultaneous,n_pol=n_pol,$
        independent_fit=independent_fit,silent=silent,max_deconvolution_components=max_deconvolution_components,catalog_file_path=catalog_file_path,$
    ...
    IF Keyword_Set(export_sim) THEN FOR fi=0L,n_files_use-1 DO BEGIN
        fhd_main,vis_file_list[fi],status_arr[fi],file_path_fhd=fhd_file_list[fi],n_pol=n_pol,/force_no_data,$
            ...
            silent=silent,max_deconvolution_components=max_deconvolution_components,deconvolve=0,catalog_file_path=catalog_file_path,$

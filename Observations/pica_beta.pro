PRO pica_beta,cleanup=cleanup,ps_export=ps_export,recalculate_all=recalculate_all,export_images=export_images,version=version,$
...
IF N_Elements(channel) EQ 0 THEN channel=117 ;options are 93, 117, or 141
...
data_directory=rootdir('mwa')+filepath('',root='DATA2',subdir=['Beta','PicA',Strn(Floor(channel))])
...
general_obs,cleanup=cleanup,ps_export=ps_export,recalculate_all=recalculate_all,export_images=export_images,version=version,$
    ...
    complex_beam=complex_beam,double_precison_beam=double_precison_beam,update_file_list=update_file_list,_Extra=extra
!except=except

pro integrate_healpix_cubes, filenames, save_file = save_file, save_path = save_path, obs_name = obs_name, $
    discard_unmatched_pix = discard_unmatched_pix, discard_unmatched_freq = discard_unmatched_freq, uvf_cubes = uvf_cubes
  ...
  if n_elements(save_file) eq 0 or n_elements(save_path) eq 0 then begin
    ...
    if n_elements(uvf_cubes) eq 1 then save_path = file_dirname(filenames[0], /mark_directory) 
  ...
  for i=0, nfiles-1 do begin
    ...
    if n_elements(dirty_uv_arr) ne 0 then if n_elements(cube_struct) eq 0 then $
          cube_struct = create_struct('dirty_uv_arr', ptr_new(temporary(dirty_uv_arr))) else $
          cube_struct = create_struct(cube_struct, 'dirty_uv_arr', ptr_new(temporary(dirty_uv_arr)))
    if n_elements(model_uv_arr) ne 0 then if n_elements(cube_struct) eq 0 then $
          cube_struct = create_struct('model_uv_arr', ptr_new(temporary(model_uv_arr))) else $
          cube_struct = create_struct(cube_struct, 'model_uv_arr', ptr_new(temporary(model_uv_arr)))
    if n_elements(variance_uv_arr) ne 0 then if n_elements(cube_struct) eq 0 then $
          cube_struct = create_struct('variance_uv_arr', ptr_new(temporary(abs(variance_uv_arr)))) else $
          cube_struct = create_struct(cube_struct, 'variance_uv_arr', ptr_new(temporary(abs(variance_uv_arr))))
    if n_elements(weights_uv_arr) ne 0 then if n_elements(cube_struct) eq 0 then $
          cube_struct = create_struct('weights_uv_arr', ptr_new(temporary(weights_uv_arr))) else $
          cube_struct = create_struct(cube_struct, 'weights_uv_arr', ptr_new(temporary(weights_uv_arr)))


    if n_elements(uvf_cubes) ne 0 then begin
      if i eq 0 then begin
        int_struct = temporary(cube_struct)
        obs_arr_use = temporary(obs_out)
      endif else begin
            obs_arr_use = [temporary(obs_arr_use), temporary(obs_out)]
      ...
      if i eq 0 then begin
        ...
        if n_elements(pixels_use) eq n_elements(hpx_inds) and count_pix_match eq n_elements(pixels_use) then begin
          ...
          if keyword_set(discard_unmatched_pix) then begin
            ...
            ;nfile_contrib_pix = intarr(n_elements(hpx_inds)) + 1
  ...
  if n_elements(uvf_cubes) ne 0 then begin
    dirty_uv_arr = *int_struct.dirty_uv_arr
    ptr_free, int_struct.dirty_uv_arr
    if tag_exist(int_struct, 'model_uv_arr') then begin
      model_uv_arr = *int_struct.model_uv_arr
      ptr_free, int_struct.model_uv_arr
      model_flag=1
    endif
    weights_uv_arr = *int_struct.weights_uv_arr
    ptr_free, int_struct.weights_uv_arr
    variance_uv_arr = *int_struct.variance_uv_arr
    ptr_free, int_struct.variance_uv_arr
    obs_out = temporary(obs_arr_use)

    if keyword_set(model_flag) then save, file = save_file, dirty_uv_arr, model_uv_arr, weights_uv_arr, variance_uv_arr, obs_out else $
      save, file = save_file, dirty_uv_arr, weights_uv_arr, variance_uv_arr, obs_out

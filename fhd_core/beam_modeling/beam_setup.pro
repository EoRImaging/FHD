FUNCTION beam_setup,obs,status_str,antenna,file_path_fhd=file_path_fhd,restore_last=restore_last,timing=timing,$
    ...
    if file_test(transfer_psf) then begin
      ...
      if file_test(transfer_psf+'/'+obs_id+'_obs.sav') AND file_test(transfer_psf+'/'+obs_id+'_beams.sav') then begin
        ...
        if file_test(transfer_psf+'/metadata/'+obs_id+'_obs.sav') AND file_test(transfer_psf+'/beams/'+obs_id+'_beams.sav') then begin 
          ...
          print, obs_id+'_obs.sav and '+obs_id+'_beams.sav in ' +transfer_psf+ ' or '
          print, 'metadata/'+obs_id+'_obs.sav and beams/'+obs_id+'_beams.sav in ' + transfer_psf
  ...
  if keyword_set(beam_function_decomp) then beam_params_arr=Ptrarr(n_pol,/allocate)
  ...
  psf=fhd_struct_init_psf(beam_ptr=0,xvals=psf_xvals,yvals=psf_yvals,fbin_i=freq_bin_i,$
    ...
    image_res_scale=image_res_scale,psf_image_resolution=psf_image_resolution,$
    beam_mask_threshold=beam_mask_threshold,pix_horizon=obs.dimension/antenna[0].psf_scale)

  if keyword_set(beam_function_decomp) then begin
    psf = structure_update(psf,_Extra={beam_decomp_info:ptr_new(beam_decomp_info)})
  endif

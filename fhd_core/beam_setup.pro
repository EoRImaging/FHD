...
IF N_Elements(psf_dim) EQ 0 THEN IF tag_exist(obs,'antenna_size') THEN $
    psf_dim=Ceil((obs.antenna_size*2.*Max(frequency_array)/speed_light)/kbinsize) ELSE psf_dim=Ceil(2.*!Pi/kbinsize) 

PRO calculate_redundant_cal_correction, vis_data, vis_model, covariance_map_fn, A_ind, B_ind, gain,$
    ...
    n_baselines = N_Elements(vis_data)/2
    FOR b_ii=0,n_baselines-1 DO BEGIN
        ; b_i = baseline_inds[b_i]
        bi_redundant = *covariance_map_fn.ija[b_ii]
        bi_red_use = where(Abs(*covariance_map_fn.sa[b_ii]) GE covariance_threshold, n_use)
        ...
        gain_A_red = gain[tile_A_redundant]
        gain_B_red = gain[tile_B_redundant]
        vis_redundant = vis_data[bi_redundant]
        model_redundant = Total(vis_model[bi_redundant])/n_bi_redundant
        new_vis_delta[baseline_inds[b_ii]] = $
            Total(vis_redundant*Conj(gain_A_red)*gain_B_red)/Total(Abs(gain_A_red)^2.*Abs(gain_B_red)^2.) - model_redundant
    ENDFOR

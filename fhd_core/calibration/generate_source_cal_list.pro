FUNCTION generate_source_cal_list,obs,psf,source_array,catalog_path=catalog_path,calibration_spectral_index=calibration_spectral_index,$
  ...
  ;If max_sources is negative, then only include the faint sources below the cut, not including the cut.
  IF Keyword_Set(max_sources) THEN BEGIN
    IF (N_Elements(source_list) GT abs(max_sources)) AND (max_sources GT 0) THEN source_list=source_list[0:max_sources-1]
    IF (N_Elements(source_list) GT abs(max_sources)) AND (max_sources LT 0) THEN source_list=source_list[abs(max_sources):N_elements(sources_list)-1]
  ENDIF

...
IF N_Elements(gain_factor) EQ 0 THEN gain_factor=.25
...
IF N_Elements(max_add_sources) EQ 0 THEN max_add_sources=Ceil(((Alog(1./beam_threshold)/obs.degpix)^2.)/10.)
IF N_Elements(max_iter) EQ 0 THEN IF max_add_sources EQ 1 THEN max_iter=max_sources ELSE max_iter=1000.
IF N_Elements(check_iter) EQ 0 THEN IF max_add_sources EQ 1 THEN check_iter=5./gain_factor ELSE check_iter=1./gain_factor

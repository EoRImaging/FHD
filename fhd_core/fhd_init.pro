FUNCTION fhd_init,obs,cal,file_path_fhd=file_path_fhd,pol_use=pol_use,freq_use=freq_use,time_i_use=time_i_use,$
    ...
    joint_deconvolution_list=joint_deconvolution_list,dft_threshold=dft_threshold,dft_kernel_threshold=dft_kernel_threshold
...
IF N_Elements(dft_threshold) EQ 0 THEN dft_threshold=1./(!Pi*dimension) ;explicitly set to 0 to use analytic DFT. ;1/2 value of kernel_test along either axis at the edge of the image. 
IF dft_threshold EQ 1 THEN dft_threshold=1./(!Pi*dimension) ELSE dft_threshold=Float(dft_threshold)
...
fhd={npol:n_pol,beam_threshold:beam_threshold,max_iter:max_iter,max_sources:max_sources,check_iter:check_iter,$
    ...
    dft_threshold:dft_threshold,independent_fit:independent_fit,$

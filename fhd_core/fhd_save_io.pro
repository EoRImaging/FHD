...
IF Keyword_Set(reset) THEN status_str={hdr:0,params:0,obs:0,beams:0,jones:0,cal:0,flags:0,autos:0,vis:intarr(4),vis_model:intarr(4),$
    uv_grid:intarr(4),uv_model_grid:intarr(4),mapfn:intarr(4),fhd:0,fhd_params:0,healpix_cube:intarr(4),hpx_even:intarr(4),hpx_odd:intarr(4)}
...
CASE var_name OF 
    'hdr':BEGIN status_use.hdr=1 & var_name='hdr' & path_add='_hdr' & subdir='metadata'& END
    'obs':BEGIN status_use.obs=1 &  var_name='obs' & path_add='_obs' & subdir='metadata'& END
    'params':BEGIN status_use.params=1 & var_name='params' & path_add='_params' & subdir='metadata'& END
    'psf':BEGIN status_use.beams=1 & var_name='psf' & path_add='_beams' & subdir='beams'& END
    'jones':BEGIN status_use.jones=1 & var_name='jones' & path_add='_jones' & subdir='beams'& END
    'cal':BEGIN status_use.cal=1 & var_name='cal' & path_add='_cal' & subdir='calibration'& END
    'flag_arr':BEGIN status_use.flags=1 & var_name='flag_arr' & path_add='_flags' & subdir='vis_data'& END
    'auto_corr':BEGIN status_use.autos=1 & var_name='auto_corr' & path_add='_autos' & subdir='vis_data' & obs_flag=1 & END
    'vis_ptr':BEGIN status_use.vis[pol_i]=1 & var_name='vis_ptr' & path_add='_vis_'+pol_names[pol_i] & subdir='vis_data' & obs_flag=1 & END
    'vis_model_ptr':BEGIN status_use.vis_model[pol_i]=1 & var_name='vis_ptr' & path_add='_vis_model_'+pol_names[pol_i] & subdir='vis_data' & obs_flag=1 & END
    'grid_uv':BEGIN status_use.uv_grid[pol_i]=1 & var_name='grid_uv' & path_add='_uv_'+pol_names[pol_i] & subdir='grid_data'& END
    'grid_uv_model':BEGIN status_use.uv_model_grid[pol_i]=1 & var_name='grid_uv' & path_add='_uv_model_'+pol_names[pol_i] & subdir='grid_data'& END
    'mapfn':BEGIN status_use.mapfn[pol_i]=1 & var_name='mapfn' & path_add='_mapfn_'+pol_names[pol_i] & subdir='mapfn'& END
    'fhd_params':BEGIN status_use.fhd_params=1 & var_name='fhd' & path_add='_fhd_params' & subdir='deconvolution'& END
...
IF ~Keyword_Set(name_error) THEN BEGIN
    ...
    IF Keyword_Set(restore) THEN BEGIN
        param=getvar_savefile(path_use,var_name)
        RETURN
    ENDIF

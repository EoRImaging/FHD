FUNCTION visibility_grid_wrap,vis_arr,vis_weights,obs,status_str,psf,params,file_path_fhd=file_path_fhd,vis_model_arr=vis_model_arr,$
    ...
    redundant_correction_flag=redundant_correction_flag, redundant_cal_correction=redundant_cal_correction,redundantCorr_uv_holo=redundantCorr_uv_holo,$
    _Extra=extra
...
IF Keyword_Set(redundant_correction_flag) THEN redundantCorr_uv_holo=Ptrarr(n_pol,/allocate)
...
FOR pol_i=0,n_pol-1 DO BEGIN
    ...
    *image_uv_arr[pol_i]=Temporary(grid_uv)
    ...
    IF Keyword_Set(redundant_correction_flag) THEN $
        *redundantCorr_uv_holo[pol_i]=Temporary(redundantCorr_return)
...
FOR pol_i=0, n_pol-1 DO BEGIN
    ...
    IF Keyword_Set(redundant_correction_flag) THEN $
        fhd_save_io,status_str,*redundantCorr_uv_holo[pol_i],var='grid_uv_redundantCorr',/compress,file_path_fhd=file_path_fhd,pol_i=pol_i,obs=obs,_Extra=extra
ENDFOR

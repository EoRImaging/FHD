FUNCTION mwa_beam_setup_init,obs,antenna,file_path_fhd=file_path_fhd,dipole_mutual_coupling_factor=dipole_mutual_coupling_factor,$
    beam_model_version=beam_model_version,xvals_instrument=xvals_instrument,yvals_instrument=yvals_instrument,$
    za_arr=za_arr,az_arr=az_arr,Jones_matrix_arr=Jones_matrix_arr,dead_dipole_list=dead_dipole_list
...
freq_center=antenna.freq
nfreq_bin=antenna.n_freq_bin
...
;        projection/=Max(projection)
    ...
    ENDELSE
ENDCASE
Jones_matrix_arr=Ptrarr(n_tiles)
FOR t_i=0L,n_tiles-1 DO Jones_matrix_arr[t_i]=Ptr_new(Jones_matrix)

...
IF overwrite_test THEN BEGIN
    ...
    layout_i=where(info_labels EQ layout_delimiter)
    ...
    IF Min([cmd_i,obs_i,antenna_i,psf_i,cal_i,fhd_i,end_i]) GE 0 THEN BEGIN
        ...
        obs_insert=info_arr[0:1,obs_i:layout_i-1]
        layout_insert=info_arr[0:1,layout_i:antenna_i-1]
...
info_out=[[info_out],[cmd_insert],[obs_insert],[layout_insert],[antenna_insert],[psf_insert],[cal_insert],[skymodel_insert],[fhd_insert],[end_delimiter,filler]]

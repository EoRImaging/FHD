...
PRO uvfits2fhd,file_path_vis,export_images=export_images,cleanup=cleanup,recalculate_all=recalculate_all,$
    ...
    weights_grid=weights_grid,save_visibilities=save_visibilities,return_cal_visibilities=return_cal_visibilities,_Extra=extra
...
IF Keyword_Set(beam_recalculate) OR Keyword_Set(grid_recalculate) OR Keyword_Set(mapfn_recalculate) OR $
   (Keyword_Set(save_visibilities) AND (vis_file_flag EQ 0)) THEN data_flag=1 ELSE data_flag=0

PRO vis_baseline_hist,obs,params,vis_ptr=vis_ptr,vis_model_ptr=vis_model_ptr,$
...
FOR pol_i=0,n_pol-1 DO BEGIN
    FOR i=0, n_elements(dist_locs)-1 DO IF dist_hist[i] GT 0 THEN BEGIN
        ...
        model_vals=(*vis_model_ptr[pol_i])[inds]
        wh_noflag = where(model_vals GT 0, count_noflag)
        IF count_noflag EQ 0 THEN CONTINUE ELSE inds = inds[wh_noflag]
        if Keyword_Set(calibration_visibilities_subtract) THEN BEGIN
            vis_res_ratio_mean[i] = mean(abs((*vis_ptr[pol_i])[inds]))/mean(abs(model_vals))
            vis_res_sigma[i] = sqrt(variance(abs((*vis_ptr[pol_i])[inds])))/mean(abs(model_vals))
        ENDIF ELSE BEGIN
            vis_res_ratio_mean[i] = mean(abs((*vis_ptr[pol_i])[inds]-model_vals))/mean(abs(model_vals))
            vis_res_sigma[i] = sqrt(variance(abs((*vis_ptr[pol_i])[inds]-model_vals)))/mean(abs(model_vals))
        ENDELSE
    ENDIF
ENDFOR

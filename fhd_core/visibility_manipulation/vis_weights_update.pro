...
FOR pol_i=0,n_pol-1 DO BEGIN
    ...
    IF Keyword_Set(no_frequency_flagging) THEN (*obs.baseline_info).freq_use=Replicate(1,n_freq) ELSE BEGIN
        ...
        IF n_freq_cut GT 0 THEN (*vis_weight_ptr[pol_i])[freq_cut_i,*]=0
    ENDELSE
    ...
    FOR ti=0L,n_time_cut-1 DO BEGIN
        ...
        IF n_ti_cut GT 0 THEN (*vis_weight_ptr[pol_i])[*,ti_cut]=0
    ENDFOR

    flag_i=where(*vis_weight_ptr[pol_i] LE 0,n_flag,ncomplement=n_unflag)
    flag_i_new=where(xmin LT 0,n_flag_new)
    IF n_flag_new GT 0 THEN (*vis_weight_ptr[pol_i])[flag_i_new]=0
    ...
    obs.n_vis[pol_i]=Total(bin_n)
    ;print, '!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!'
    print, 'Setting n_vis to total(bin_n): '
    print, Total(bin_n)
ENDFOR

        ...
        FOR bin_i=0L,memory_bins-1 DO BEGIN
            ...
            IF keyword_set(gaussian_source_models) THEN BEGIN
              if n_elements(gaussian_rot) gt 0 then begin
                source_envelope = exp(-Pi^2./(4.*alog(2)) * $
                ...
                source_envelope = exp(-Pi^2./(4.*alog(2))*(matrix_multiply(xvals^2., gauss_x_use[inds]^2.)+matrix_multiply(yvals^2., gauss_y_use[inds]^2.)))
              endelse
    ...
    ENDIF ELSE BEGIN
      ...
      IF keyword_set(gaussian_source_models) THEN BEGIN
        if n_elements(gaussian_rot) gt 0 then begin
          ...
          source_envelope = exp(-Pi^2./(4.*alog(2))*(matrix_multiply(xvals^2., gauss_x_use^2.)+matrix_multiply(yvals^2., gauss_y_use^2.)))
        endelse
        cos_term *= source_envelope
        sin_term *= source_envelope
      ENDIF

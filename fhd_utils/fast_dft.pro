FUNCTION fast_dft,x_vec,y_vec,dimension=dimension,elements=elements,degpix=degpix,flux_arr=flux_arr,$
    conserve_memory=conserve_memory,dft_approximation_resolution=dft_approximation_resolution,dft_kernel_threshold=dft_kernel_threshold,_Extra=extra
...
FOR pol_i=0,n_pol-1 DO BEGIN
    model_img=fast_dft_subroutine(x_vec,y_vec,*flux_arr_use[pol_i],dft_approximation_resolution=dft_approximation_resolution,$
        dft_kernel_threshold=dft_kernel_threshold,dimension=dimension,elements=elements,conserve_memory=conserve_memory)

FUNCTION healpix_combine_inds,hpx_cnv,hpx_inds=hpx_inds,full_ind_reference=full_ind_reference
...
hpx_ind_map=Ptrarr(size(hpx_cnv,/dimension),/allocate)
FOR obs_i=0,n_obs-1 DO BEGIN
    ind_use2=where((*hist_arr[obs_i])[ind_use],n_use2)
    *hpx_ind_map[obs_i]=ind_use2
    *hist_arr[obs_i]=0  ;free memory
ENDFOR
Ptr_free,hist_arr

IF Arg_Present(full_ind_reference) THEN BEGIN
    full_ind_reference=L64indgen(hpx_max-hpx_min+1)-1

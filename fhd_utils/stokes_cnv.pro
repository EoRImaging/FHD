...
ENDIF ELSE BEGIN
    ...
    FOR ii=0L,n_dims-1 DO *beam_use[ii]=*beam_arr[ii]
    
    IF ~Keyword_Set(inverse) THEN BEGIN
        IF ~Keyword_Set(p_corr) THEN BEGIN 
            p_use=Ptrarr(dims,/allocate) 
            FOR ii=0L,n_dims-1 DO *p_use[ii]=1. 
    ...
    ENDELSE 
    IF Keyword_Set(square) THEN FOR ii=0L,n_dims-1 DO *beam_use[ii]=*beam_use[ii]^2.
    ...
    IF ~Keyword_Set(inverse) THEN BEGIN
        image_arr_out[0]=Ptr_new(((*image_arr[stokes_list1[pol_i]])*(*p_use[stokes_list1[pol_i]])+$
            (*image_arr[stokes_list2[pol_i]]*(*p_use[stokes_list2[pol_i]])))*weight_invert((*beam_use[stokes_list2[pol_i]])+(*beam_use[stokes_list1[pol_i]]))/2.)
    ENDIF ELSE BEGIN
        image_arr_out[0]=Ptr_new(((*image_arr[stokes_list1[pol_i]])*(*p_use[stokes_list1[pol_i]])+$

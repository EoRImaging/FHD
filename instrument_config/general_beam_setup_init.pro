FUNCTION general_beam_setup_init,obs,antenna_str,_Extra=extra
  ...
  case obs.instrument of
    "hera": IF N_Elements(antenna_size) EQ 0 THEN antenna_size=14 ;HERA dish diameter is 14m
    "mwa": IF N_Elements(antenna_size) EQ 0 THEN antenna_size=5. ;meters (MWA groundscreen size)
    "lwa": IF N_Elements(antenna_size) EQ 0 THEN antenna_size=3. ;meters (LWA groundscreen size)
  endcase
  ...
  if n_elements(antenna_size) gt 0 then begin

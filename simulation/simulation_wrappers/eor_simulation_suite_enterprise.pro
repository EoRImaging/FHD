pro eor_simulation_suite_enterprise, uvf_input = uvf_input, flat_sigma = flat_sigma, run_name = run_name, $
  ...
  sim_ave_powers_uvf = fltarr(nbeams, n_elements(sample_factors))
  sim_wt_ave_powers_uvf = fltarr(nbeams, n_elements(sample_factors))
  ...
  if not keyword_set(no_suite_plots) then begin
    ...
    if keyword_set(plot_ratio) then begin
      ...
      yrange=minmax([sim_ave_powers, sim_wt_ave_powers, + fltarr(nbeams, n_elements(sample_factors)) + flat_power])
    ...
    if keyword_set(plot_ratio) then begin
      ...
      ;yrange=minmax([sim_ave_powers_uvf, sim_wt_ave_powers_uvf, + fltarr(nbeams, n_elements(sample_factors)) + flat_power])
      yrange=minmax([sim_ave_powers_uvf, + fltarr(nbeams, n_elements(sample_factors)) + flat_power])
      yrange = 10^float([floor(alog10(yrange[0])), ceil(alog10(yrange[1]))])
      xrange=minmax(sim_nbsl_lambda2)
